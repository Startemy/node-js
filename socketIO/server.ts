...
const userCount: Record<string, any> = {}
let userName = [{
  id: String,
  nikName: String
}]
...
socket.on('connection', function (client: any) {
  ...
  client.on('CLIENT_MSG_NAME', (data: any) => {
    client.broadcast.emit('NEW_CONNECT', { msg: 'The new client connected: ', name: data.nikName })
    userName = [...userName, { id: client.id, nikName: data.nikName }]
  })

  client.on('CLIENT_MSG', (data: any) => {
    userName.forEach((nikName) => {
      if (nikName.id == client.id) {
        client.broadcast.emit('SERVER_MSG', { msg: data.msg, clName: nikName.nikName });
  ...
  client.on('disconnect', () => {
    ...
    userName.forEach((nikName) => {
      client.broadcast.emit('SERVER_MSG', { msg: 'отключился от чата', clName: nikName.nikName });
    })
  })
});
